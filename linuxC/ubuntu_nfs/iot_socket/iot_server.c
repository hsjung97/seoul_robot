...
void * clnt_connection(void *arg)
{
		...
		while(1)
		{
               ...
               // [추가된 핵심 로직] 
               // 만약 아두이노(BTARD)가 메시지를 보낸 것이라면, 
               // 누가 시켰든 상관없이 무조건 목적지(to)를 'HSJ_SQL'로 고정합니다.
                if (strcmp(client_info->id, "HSJ_BTARD") == 0) {
                  msg_info.to = "HSJ_SQL"; 
                  sprintf(to_msg, "[HSJ_SQL]%s", pArray[1]);
                } else {
                // 그 외(PHP 등)의 경우에는 원래대로 파싱된 대상을 목적지로 설정
                  msg_info.to = pArray[0];
                  sprintf(to_msg, "[%s]%s", msg_info.from, pArray[1]);
                }
